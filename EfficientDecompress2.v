...
(* note that this is not fully verified, just for testing *)
Program Fixpoint ReadManyBlocks (n : nat) (l : LB) {measure (ll l)} : SequenceWithBackRefs Byte :=
  match l with
    | [] => []
    | (endb :: l') =>
...
Definition DeflateTest (l : LB) :=
  map Vector.to_list (thawBL (@BackRefs Byte (d"32768") (ReadManyBlocks 0 l) (nilBL _ _) (Bvector.Bvect_false _))).
...
(* Our more efficient Lists 
Extract Inductive list => "Okasaki.CatDeque" [ "Okasaki.cdNil" "Okasaki.cdCons" ] "Okasaki.cdRec".

Extract Constant app => "Okasaki.cdConcat".

Extract Constant length => "Okasaki.cdLength".

Extract Constant resolveNthLast => "Okasaki.myResolveNthLast". *)

Extract Inductive list => "[]" [ "[]" "(\ a b -> a : b)" ]
                               "(\ n c l -> case l of { [] -> n [] ; (b : bs) -> c b bs })".

Extraction "Test3.hs" DeflateTestNoBackRefs.
